...
namespace ECommerce.Controllers
{
    public class UsersController : Controller
    {
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.CompanyId = new SelectList(CombosHelpers.GetCompanys(), "CompanyId", "Name");
            ViewBag.DepartamentsId = new SelectList(CombosHelpers.GetDepartments(), "DepartamentsId", "Name");
            return View();
        }
        ...
        public ActionResult Create(User user)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user.PhotoFile != null)
                {
                    ...
                    if (response)
                    {
                        pic = string.Format("{0}/{1}.jpg", folder, file, user.UserId);
                        user.Photo = pic;
            ...
            ViewBag.CompanyId = new SelectList(CombosHelpers.GetCompanys(), "CompanyId", "Name", user.CompanyId);
            ViewBag.DepartamentsId = new SelectList(CombosHelpers.GetDepartments(), "DepartamentsId", "Name", user.DepartamentsId);
            return View(user);
        }
        ...
        public ActionResult Edit(User user)
        {
            if (ModelState.IsValid)
            {
                if (user.PhotoFile != null)
                {
                    ...
                    if (response)
                    {
                        pic = string.Format("{0}/{1}", folder, file, user.CompanyId);
                        user.Photo = pic;
                    }
                }
