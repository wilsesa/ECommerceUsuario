    ...
    public class DepartamentsController : Controller
    {
        ...
        public ActionResult Edit([Bind(Include = "DepartamentsId,Name")] Departaments departaments)
        {
            if (ModelState.IsValid)
            {
                ...
                catch (System.Exception ex)
                {
                    if (ex.InnerException != null && ex.InnerException.InnerException != null && ex.InnerException.InnerException.Message.Contains("_Index"))
                    {
                        ModelState.AddModelError(string.Empty, "Não é possível atualizar dois departamentos com o mesmo nome !!");
                    }
                    else
                    {
                        ModelState.AddModelError(string.Empty, ex.Message);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            catch (System.Exception ex)
            {
                ...
                {
                    ModelState.AddModelError(string.Empty, "Não é possível remover o departamento porque existe cidades relacionadas a ele, primeiro remova as cidades e volte a excluir!!");
                }
                else
                {
                    ModelState.AddModelError(string.Empty, ex.Message);
                    //ModelState.AddModelError(string.Empty, "Erro 2");
                }
